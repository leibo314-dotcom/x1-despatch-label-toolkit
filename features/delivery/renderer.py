#!/usr/bin/env python3
"""Existing docket renderer, isolated from all validation features."""
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        for page_index, page in enumerate(pdf.pages):
            ...
            if document_type == 'assembly':
                # Screw descriptions contain '#2'; item headings start a line.
                starts = {id(line[0]) for line in group_words_by_line(words) if line}
                headers = [w for w in headers if id(w) in starts]
            ...
            for i, w in enumerate(headers):
                ...
                frame = ""
                m_frame = (re.search(r"(?m)^Frame\s*:\s*([^\n]+)", seg)
                           if document_type == 'assembly' else
                           re.search(r"\n([^\n]*\([^\n]*\)[^\n]*)\n", seg))
                if m_frame:
